...
NOMINAL_FOOT_OFFSETS = (
    (0.2670, 0.2558, -0.3364),
    (0.2670, -0.2558, -0.3364),
    (-0.3860, 0.2558, -0.3364),
    (-0.3860, -0.2558, -0.3364),
)
...
def stand_still(context: VelocityRewardContext, reward_config: Any) -> jax.Array:
    """Penalize deviating from the default stance at a zero command while the
    robot is (nearly) stationary -- M20 style.

    The policy cannot observe its own linear velocity (``base_lin_vel`` is fed
    as zeros), so a raw velocity penalty is unlearnable AND would charge the
    physical deceleration transient after a command snaps to zero.  Instead we
    mirror deeprobotics' M20: gate on the REAL body speed (rewards see true
    velocities even though the policy obs feeds 0), and only when BOTH the
    command is ~zero AND the body is not actually moving do we penalize
    (a) leg joints that deviate from default stance and (b) wheel spin.  A
    robot coasting to a stop stays off-penalty; a robot standing still but
    creeping pays.
    """
    ...
    # Stationary gate on the REAL planar body speed: coasting-to-stop is free.
    vel_threshold = getattr(reward_config, "stand_still_vel_threshold", 0.3)
    base_speed = jp.linalg.norm(context.base_lin_vel[:2])
    is_stationary = (base_speed < vel_threshold).astype(jp.float32)
    active = is_zero_cmd * is_stationary

    # (a) Leg joints back to default stance (L1), policy-order first 12.
    n = context.num_position_joints if context.num_position_joints is not None else 12
    leg_err = jp.sum(jp.abs(context.joint_pos[:n] - context.default_pose[:n]))

    # (b) Wheels must not spin while standing (L1).
    ...
    if context.joint_vel is not None and context.num_position_joints is not None:
        ...
        wheel_pen = jp.sum(jp.abs(wheel_vel))

    wheel_weight = getattr(reward_config, "stand_still_wheel_weight", 0.05)
    return (leg_err + wheel_weight * wheel_pen) * active
