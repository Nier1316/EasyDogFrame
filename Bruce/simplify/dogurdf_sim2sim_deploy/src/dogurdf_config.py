...
def get_dogurdf_config():
    ...
    # Leg gains (aligned with real machine kp=250, 2026-08-21): at 56.71 kg the
    # pure-PD stance sags ~4 cm with 250 N.m torque limit (peak ~48 N.m).  kp
    # was 150 during the 14 kg-era; raised to 250 to match the deployment rig.
    config.control.stiffness = 250.0
    ...
    # 0.2 (raised from 0.01 on 2026-08-21) proved too aggressive: it dominated
    # the reward (stand_still term hit -0.2 vs all positive terms <0.07) and
    # froze the policy under zero command, crashing the gait. Dialed to 0.05.
    config.rewards.stand_still_wheel_weight = 0.05
    # M20-style (deeprobotics): gate the stand_still penalty on the REAL body
    # speed (rewards see true velocities even though the policy obs feeds 0).
    # Only penalize when the body is (nearly) stationary, so the deceleration
    # ramp after command->0 is free; stand_still then penalizes leg joints
    # off-default + wheel spin.
    config.rewards.stand_still_vel_threshold = 0.3   # m/s (planar, below = stationary)
    ...
    config.rewards.scales.stand_still = -1.0
    ...
    # 2026-08-21: 3.0 -> 2.0 and lift_turn_target 0.12 -> 0.09 to tame swing
    # amplitude during step-turn (user reported legs swing too wide).
    config.rewards.scales.feet_lift_turn = 2.0
    config.rewards.lift_turn_target = 0.09   # clearance (m) above ground where lift reward saturates
    ...
    # Raised 10 -> 40 N on 2026-08-21: threshold was set in the 14.22 kg era;
    # at 56.71 kg a non-foot limb brushing the ground easily exceeds 10 N (and
    # friction 1.36 amplifies it), terminating episodes before a gait can form.
