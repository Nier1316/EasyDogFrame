...
static const MotorLimits MOTOR_LIMITS[MOTOR_TYPE_NUM] = {
	...
	//   2026-09-04 提至：Hip(侧摆)/Thigh(髋)=±120、Calf(膝)=±200、Wheel=±52。
	//   编码下发与反馈解码都按此解析（需固件侧同步改 ±120，否则量程错位）；
	//   命令扭矩另经 TORQUE_CMD_LIMIT clamp（float_to_uint 无 clip，防越界）。
	/* Hip   */ {-12.5f, 12.5f,  -3.0f,  3.0f, -120.0f, 120.0f, 0.0f, 500.0f, 0.0f, 100.0f, 0.0f, 500.0f},
	/* Thigh */ {-12.5f, 12.5f,  -3.0f,  3.0f, -120.0f, 120.0f, 0.0f, 500.0f, 0.0f, 100.0f, 0.0f, 500.0f},
...
// 命令扭矩上限（真机保护，2026-08-30 用户改，2026-09-04 Hip/Thigh 110→120，与 MOTOR_LIMITS 一致）：
//   Hip/Thigh=120、Calf=200、Wheel=52。编码前 clamp 命令扭矩，防 float_to_uint 越界
