/**
 * @file    types.h
 * @brief   CAN 电机控制框架的通用数据类型定义（L0 公共基础）
 * @details 本文件定义框架使用的公共数据结构与常量：通用 CAN 帧、电机状态、CAN 设备配置。
 *          上层模块（CanTransport / MotorManager / RobotApp）依赖本文件。
 */
#ifndef TYPES_H_
#define TYPES_H_
...
#include "motor/ele_motor_def.h"  // 电机定义（包括 ControlMode）
...
/**
 * @brief 传输层无关的 CAN 帧表示（隐藏具体后端：CANET / SocketCAN / ttyACM）
 * @note  CanetTransport 中原 BspCanFrame 即此类型（using 别名兼容旧调用点）。
 */
struct CanFrame {
    uint32_t id;           // CAN ID
