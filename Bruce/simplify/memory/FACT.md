...
- 电机环频率（2026-08-28 最新）：`motor_receive` = 2ms（500Hz）、`motor_send` = 2ms（500Hz，原 1ms 冗余已改），`robot_calibration.h` `CONTROL_HZ=500`。轮子上位机 500Hz 速度环也在 SendOnce。
...
- 关键常量（**2026-08-28 对齐 v28 sim2sim.py 默认**）：64 维观测 / 16 维动作、`ACTION_SCALE=0.25`、**`LEG_KP=250`、`LEG_KD=4`、`WHEEL_KD=2.0`、`LEG_TORQUE_LIMIT=250`**（全部 = `RL_Train/code/src/sim2sim.py` 默认，**traj_v28 训练 stiffness=250/damping=4**，见 RL_Train/code commit 07884c7 回退 v28 默认；⚠ 300/10 是 V30 参数勿混淆）、`WHEEL_VEL_SCALE=12.5`、`WHEEL_TORQUE_LIMIT=53`、`GAIT_CYCLE=0.6`、`GAIT_OFFSET={0,0.5,0.5,0}`、`CONTROL_DT=0.02`（RL 50Hz）。⚠ 历史：曾用 LEG_KP/KD=250/40（真机标定）、WHEEL_KD=1.0（2→1 抑振荡）、LEG_TORQUE_LIMIT=150，用户决定统一套 v28 sim2sim 默认。
- 站立稳定（2026-08-28）：`JOINT_IMPEDANCE` = hip kp300/kd10/**tau_ff+10**、thigh/calf kp250/kd10；`CMD_BIAS_VX=-0.05`（Example36 + MotionController `cmd_bias_vx`）抵消策略 wheel action 正向偏置（整体前冲）。
...
- 示例分工（ex_rl.cpp + ex_diag.cpp）：`Example25` 完整 RL 循环（50Hz、手柄、急停）；`Example30` 离线链路回归；`Example31` 零位对齐/关节范围扫描（不使能电机）；`Example32` 默认姿态验证；`Example35` 轮摩擦前馈标定；`Example36` 4 路 USB2CAN RL 站立循环（**当前 main.cpp 激活**）；`Example37` 手柄遥操作（走 MotionController，补了 SetTransport(USB2CAN)）；`Example38` 动作延迟测量；`Example47` 整狗站立 chirp 参数辨识（sysid_all.csv）；`Example48` 轮子扭矩方向验证。
...
- 最新提交（2026-08-28）：a0327c8 参数辨识（Example47 整狗 chirp）+ 轮子 500Hz 上位机速度环闭环改造 + motor_send 1ms→2ms + USB2CAN recv 取空队列 + 轮速低通滤波；6e9877b 站立稳定（JOINT_IMPEDANCE tau_ff/KP/KD 优化、CMD_BIAS_VX 前冲补偿、WHEEL_KD 2→1、Example48 轮子方向验证）。新增 `docs/RL_TRAINING_REFERENCE.md`（真机辨识参数/控制架构/KP/KD/tau_ff 建议）。
